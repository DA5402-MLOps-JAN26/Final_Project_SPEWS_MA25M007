...
def compute_weekly_vle_features(vle_df: pd.DataFrame, week_num: int) -> pd.DataFrame:
    """Compute VLE-based features for all students up to given week."""
    df = vle_df[vle_df['week'] <= week_num].copy()

    # Weekly click count for current week
    current = df[df['week'] == week_num].groupby('id_student')['sum_click'].sum()
    current = current.reset_index().rename(columns={'sum_click': 'weekly_clicks_current'})

    # Cumulative click count up to this week
    cumul = df.groupby('id_student')['sum_click'].sum()
    cumul = cumul.reset_index().rename(columns={'sum_click': 'cumulative_clicks'})

    # Weeks since last active
    last_active = df.groupby('id_student')['week'].max()
    last_active = last_active.reset_index().rename(columns={'week': 'last_active_week'})
    last_active['weeks_since_active'] = week_num - last_active['last_active_week']

    # Click trend: slope over last 3 weeks
    ...
    slopes = weekly_agg.groupby('id_student').apply(click_slope, include_groups=False).reset_index()
...
def compute_assessment_features(assess_df: pd.DataFrame, assess_meta: pd.DataFrame, week_num: int) -> pd.DataFrame:
    """Compute assessment-based features up to given week."""
    ...
    latest = merged.sort_values('date').groupby('id_student').last()[['score']].reset_index()
    latest.rename(columns={'score': 'latest_score'}, inplace=True)

    # Average weighted score – handle zero weights safely
    def weighted_avg(group):
        scores = group['score'].fillna(0)
    ...
    submitted = merged.groupby('id_student')['id_assessment'].nunique().reset_index()
    submitted.rename(columns={'id_assessment': 'submitted_count'}, inplace=True)
    submitted['missed_assessments'] = all_due - submitted['submitted_count']

    # Days late on last submission
    merged['due_date'] = merged['date']
    merged['days_late'] = (merged['date_submitted'] - merged['due_date']).clip(lower=0)
    late = merged.groupby('id_student')['days_late'].mean().reset_index()
    late.rename(columns={'days_late': 'avg_days_late'}, inplace=True)
    ...
    features = features.fillna({'latest_score': 0, 'avg_weighted_score': 0,
                                'missed_assessments': 0, 'avg_days_late': 0})
    return features

def compute_demographic_features(info_df: pd.DataFrame) -> pd.DataFrame:
    """Encode demographic features from studentInfo."""
    df = info_df[['id_student', 'gender', 'highest_education', 'imd_band',
    ...
    return df[['id_student', 'gender_enc', 'disability_enc', 'education_level',
               'imd_score', 'num_of_prev_attempts', 'studied_credits']]

def compute_registration_features(reg_df: pd.DataFrame, week_num: int) -> pd.DataFrame:
    """Early unregistration flag - strongest single signal."""
    df = reg_df[reg_df['code_module'] == 'BBB'].copy()
    df['early_unreg'] = (
        df['date_unregistration'].notna() &
        (df['date_unregistration'].astype(float) < week_num * 7)
    ).astype(int)
...
def build_feature_matrix(info_df, vle_df, assess_df, assess_meta, reg_df, week_num: int) -> pd.DataFrame:
    """Assemble complete feature matrix for given week."""
    vle_df = vle_df.copy()
