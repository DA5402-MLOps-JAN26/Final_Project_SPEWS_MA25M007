...
# # ------------------------------------------------------------------
# # Screen 1: Student Risk Dashboard (Grid Layout)
# # ------------------------------------------------------------------
# if page == "Student Risk Dashboard":
#     st.title("🎓 Student Risk Dashboard")

#     # --- Top Section: 3 columns x 4 rows of inputs ---
...
#         clicks = st.number_input("Weekly Clicks", value=12.0, step=1.0)
#         cum_clicks = st.number_input("Cumulative Clicks", value=72.0, step=1.0)

#     with col2:
#         latest_score = st.number_input("Latest Score", value=34.0, step=1.0)
#         avg_score = st.number_input("Avg Weighted Score", value=34.0, step=1.0)
#         missed = st.number_input("Missed Assessments", value=2, step=1)
#         days_late = st.number_input("Avg Days Late", value=5.0, step=0.5)

#     with col3:
#         weeks_inactive = st.number_input("Weeks Inactive", value=0.0, step=1.0)
#         click_trend = st.number_input("Click Trend Slope", value=-2.0, step=0.5)
#         prev_attempts = st.number_input("Previous Attempts", value=0, step=1)
#         early_unreg = st.selectbox("Early Unregistered?", ["No", "Yes"])

#     st.markdown("---")

#     # --- Bottom Section: Trend chart (left) | Prediction (right) ---
...
# # ------------------------------------------------------------------
# # Screen 2: Cohort Overview (unchanged)
# # ------------------------------------------------------------------
...
# # ------------------------------------------------------------------
# # Screen 3: ML Pipeline Console (unchanged)
# # ------------------------------------------------------------------
...
# # ------------------------------------------------------------------
# # Screen 4: Monitoring Dashboard (Improved Metrics)
# # ------------------------------------------------------------------
...
#             if spews_lines:
#                 # Display in a clean code block
#                 st.code("\n".join(spews_lines[:20]), language="text")
#             else:
#                 st.info("No SPEWS metrics collected yet. Make some prediction requests.")
#         else:
